...
@test "($PLUGIN_COMMAND_PREFIX:link) exports MEMCACHED_URL to app" {
  ...
  assert_contains "$url" "dokku-memcached-ls:11211"
  assert_success
  dokku "$PLUGIN_COMMAND_PREFIX:unlink" ls my-app
}

@test "($PLUGIN_COMMAND_PREFIX:link) generates an alternate config url when MEMCACHED_URL already in use" {
  dokku config:set my-app MEMCACHED_URL=host:11211
  dokku "$PLUGIN_COMMAND_PREFIX:link" ls my-app
  run dokku config my-app
  assert_contains "${lines[*]}" "DOKKU_MEMCACHED_AQUA_URL"
  assert_success

  dokku "$PLUGIN_COMMAND_PREFIX:link" ms my-app
  run dokku config my-app
  assert_contains "${lines[*]}" "DOKKU_MEMCACHED_BLACK_URL"
  assert_success
...
@test "($PLUGIN_COMMAND_PREFIX:link) links to app with docker-options" {
  ...
  assert_contains "${lines[*]}" "--link dokku.memcached.ls:dokku-memcached-ls"
  assert_success
  dokku "$PLUGIN_COMMAND_PREFIX:unlink" ls my-app
}

@test "($PLUGIN_COMMAND_PREFIX:link) uses apps MEMCACHED_DATABASE_SCHEME variable" {
  dokku config:set my-app MEMCACHED_DATABASE_SCHEME=memcached2
  dokku "$PLUGIN_COMMAND_PREFIX:link" ls my-app
  url=$(dokku config:get my-app MEMCACHED_URL)
  assert_contains "$url" "memcached2://dokku-memcached-ls:11211"
  assert_success
  dokku "$PLUGIN_COMMAND_PREFIX:unlink" ls my-app
}
