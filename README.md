...
memcached:unmount [--all] <service> [<source:container-dir>...] # remove one or all mounts from the service container
...
An image other than memcached has no version to fall back on, because the version this plugin pins belongs to memcached, so name one alongside it.

```shell
dokku memcached:create lollipop --image <image> --image-version <version>
```
...
```shell
dokku memcached:create lollipop --log-opt max-size=20m,max-file=3
```
...
Alongside the connection information this reports the properties set on the service, the state it was created with, and its backup settings. A property that was never set, or that was unset, reports as empty. Omit the service to report on every memcached service:

```shell
dokku memcached:info
```

The information can be read by machine, one json object per service:

```shell
dokku memcached:info lollipop --format json
```
...
The properties memcached:set writes are reported under the names it takes, so a value read here can be written back:
...
An app is still linked after its `MEMCACHED_URL` is changed to point elsewhere, and is unlinked the same way. The variable it now holds is not the service's, so it is left alone, nothing is unset, the app is not restarted, and a warning says so.
...
```shell
# usage
dokku memcached:mount [--replace] <service> <source:container-dir[:options]>...
```
...
```shell
dokku memcached:mount lollipop /var/lib/dokku/data/storage/lollipop:/opt/extra --volume-readonly --volume-options z
```
...
```shell
dokku memcached:mount --replace lollipop /srv/a:/opt/a:ro /srv/b:/opt/b
```

> NOTE: a mount reaches the container the next time one is built. memcached:restart keeps the container it has, so use memcached:stop and then memcached:start on a service that is already running.

### remove one or all mounts from the service container

```shell
# usage
dokku memcached:unmount [--all] <service> [<source:container-dir>...]
```
...
```shell
dokku memcached:unmount lollipop /var/lib/dokku/data/storage/lollipop:/opt/extra
```

Remove every mount the service has:

```shell
dokku memcached:unmount --all lollipop
```

> NOTE: the mount is removed from the container the next time one is built. memcached:restart keeps the container it has, so use memcached:stop and then memcached:start on a service that is already running.
...
A service comes back on the version it was created with, or was last upgraded to, whatever version the plugin ships now. The image is fetched if the host no longer has it. A service that has never recorded a version and has no container left to read one from cannot be placed, and is reported rather than started on a guess. Use memcached:upgrade to say which version it should run.
...
```shell
dokku memcached:upgrade lollipop --image-version 1.2.3
```
